...
- ts: timestamp, UTC, required.
- region: RegionCode, required.
- city: string, required, one of the region's configured sampling points (E-7). The contract freezes the comparison at city granularity (E-9, E-10), so the model side has to carry the sampling point it was fetched for: a station is compared against the model value for its own city, and only then aggregated to the region-window. Without it the model half of a city-window is not recoverable from the emitted stream, and a station could be compared against a value fetched for a different city in the same region. The field follows mechanically from a frozen rule, as E-4's third member and model_pm25_ugm3 above did. E-2 deliberately does not carry it: nothing is compared against the weather stream and the index reads it at region granularity.
...
- Sources: the simulated source generates one WeatherEvent per region and one SatelliteEvent per configured city per tick, and is the default so that the local quickstart and the smoke checks stay offline and deterministic. The real source fetches readings for the configured representative cities of each region (E-7) and emits, per city, one WeatherEvent and one SatelliteEvent keyed by that city's region, so a window normally holds several events per region. The simulated satellite stream is per city because E-3 now carries the sampling point and a generated event may not be attributed to a city it was not generated for; matching the real source's composition also keeps a window computed on the simulated feed structurally the window computed on the real one. A region with no configured city is refused at construction by both sources rather than emitted as a window with no satellite event, because an empty city list would read as thinned coverage rather than as a misconfiguration.
