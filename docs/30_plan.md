...
- one seeded violation for the widened validation gate (FR-3, INV-3, E-4), in which a malformed station payload and an envelope carrying an event type outside the declared set are each fed to the gate and must quarantine with a reason code rather than passing through. The gate was written when the envelope had two members, and a gate that silently admits an unrecognised type is a gate that cannot fail on the case it was widened for. AT-2 covers the two original types; this extends the same proof to the third rather than assuming it generalised.
